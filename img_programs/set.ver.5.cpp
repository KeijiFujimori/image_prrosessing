  ...
  for(int i = 0; i < 3; ++i){
    ...
    for(int k = 0; k < 8; ++k){
      ...
      // 画像を格納するオブジェクトを宣言する
	    ...
	    cv::LUT(dst, cv::Mat(1, 256, CV_8UC1, lut), dst2);	// ルックアップテーブル変換
      j++;
      ...
      /*
      CommandLineParser parser( argc, argv, save_path(j) );
      Mat image = imread( parser.get<String>( save_path(j) ) );
      if( image.empty() )
      {
    ...
    cv::imwrite(save_path(j), new_image); // 画像を保存
      ...
      for (int i = 0; i < 256; i++) {
	    ...
	    cv::LUT(revasal, cv::Mat(1, 256, CV_8UC1, lut), dst2);	// ルックアップテーブル変換
      j++;
      cv::imwrite(save_path(j), dst2); // 画像を保存
